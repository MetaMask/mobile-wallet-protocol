...
export interface DappClientOptions {
	relayUrl: string;
	kvstore?: IKVStore; // Make it optional and default to in-memory
...
export class DappClient extends BaseClient {
	...
	public static async create(options: DappClientOptions): Promise<DappClient> {
		...
		return new DappClient(transport, new KeyManager());
	}
